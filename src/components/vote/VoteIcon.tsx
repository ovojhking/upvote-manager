...
const unselectIconColor = "#343A40";
const selectIconColor = "#253CF2";
const unselectBgColor = "#F4F6F8";
const selectBgColor = "#E5E8FD";

function VoteIcon(props: {subIndex: number, data: voteItem, handleClick: (subIndex: number) => void}) {
    const {subIndex, data, handleClick} = props;

    return (
        <button onClick={()=>handleClick(subIndex)} style={{backgroundColor: `${data.status ? selectBgColor : unselectBgColor}`}} className="mx-[5px] my-[5px] rounded-[3px] cursor-pointer">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path 
                    ...
                    fill={data.status ? selectIconColor : unselectIconColor}
                />
            </svg>
        </button>
    );
